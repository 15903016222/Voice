...
void NetworkManager::get_local_ip_address(QString &ip)
{
...
bool NetworkManager::set_ip_address(const QString &newIp)
{
    if(newIp.isEmpty()) {
        return false;
    }
...
void NetworkManager::get_subnet_mask(QString &mask)
{
    int fd;
    char netMask[50];
...
bool NetworkManager::set_subnet_mask(const QString &mask)
{
    if(mask.isEmpty()) {
        return false;
    }
