...
void TcgItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    for (int i = 0; i < pointQty-1; ++i) {
        painter->drawLine(xRatio*tcg->position(i)+3, m_size.height() - yRatio * tcg->amplitude(i) - 3,
                          xRatio*tcg->position(i+1)+3, m_size.height() - yRatio * tcg->amplitude(i+1) - 3);
    }
    ...
    for (int i = 0; i < pointQty; ++i) {
        painter->drawRect(xRatio*tcg->position(i),
                          m_size.height() - yRatio*tcg->amplitude(i) - 5,
                          6,
                          6);
    }
    painter->setBrush(Qt::red);
    painter->drawRect(xRatio*m_tcgs->position(),
