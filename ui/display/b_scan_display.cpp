...
static const int TIME_OUT_VALUE = 100;
...
bool BscanDisplay::set_current_beam(int index)
{
    if(index < 0 || m_group->beams()->beam_qty() == 0) {
        return false;
    }
...
bool BscanDisplay::set_scan_type(BscanDisplay::E_SCAN_TYPE type)
{
    ...
    if(m_type == TIME) {
        ...
        //m_bscanScene = new BscanEncoderScene(DplDevice::Device::instance()->display()->palette());
    } else {
        m_bscanScene = NULL;
        return false;
    }
...
//    DplSource::Source::instance();
    int PRF = 20;

    qDebug() << "[" << __FUNCTION__ <<  "]" << "beam qty = " << DplDevice::Device::instance()->total_beam_qty();


    m_scanTypeRulerStart = 0.0;
//    m_scanTypeRulerEnd   = size.width() / m_pixPerBeam * DplSource::Source::instance()->interval();
    m_scanTypeRulerEnd   = size.width()  / PRF;
